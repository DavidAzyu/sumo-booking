...
`📅 *Date:* ${selectedSumo.date} | ⏰ ${selectedSumo.time}%0A%0A`+
`💺 *SEAT DETAILS:*%0A`+
`${selectedSeats.map(s=>`  • Seat No. ${s} - ${getSeatLabel(s)}`).join('%0A')}%0A`+
`Visual: ${seatVisual}%0A%0A`+
`💰 *Total:* Rs.${total} (${selectedSeats.length} x ${selectedSumo.price})%0A%0A`+
`👤 *Passenger:* ${name}%0A`+
`📱 *Phone:* ${phone}%0A`+
`🏦 *UTR:* ${utr}%0A%0A`+
`✅ *Please confirm this booking!*%0A`+
`Siaha One Stop Solution - 9362600601`;
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fb] font-sans">
      ...
      <div className="import { useState, useEffect } from 'react';
...
text-center py-10 text-xs text-gray-400">© Siaha One Stop Solution • Admin: password 9362</div>
    </div>
  );
}
